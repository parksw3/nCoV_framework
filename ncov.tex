...
however, their modeling approaches and the resulting estimates vary widely, despite relying on similar data sources. \bmb{why is this a problem/opportunity? ``however'' suggests some sort of problem, but it isn't explicitly stated what it is \ldots}
% don't need {\ } after \Ro because we use xspace
...
While rapid response during an outbreak is important, careful consideration of methodology is also worthwhile. 
Modelers should work with field-workers to develop better methods for characterizing generation intervals.
\dc{This implies it is not currently the case but we do not give any justification for this assertion in this paper... that may be unfair. I know people in the UK put a lot of effort into this for Ebola. Maybe you have the same people in China doing the same thing. My point is that we should say it is not currently shared publicly.}
\swp{Modelers should continue to work closely with field-workers to develop better methods for characterizing generation intervals and make their analyses and data publicly available?}
\bmb{The last two sentences seem disconnected from the rest. What was the point you wanted to make? (I think the abstract would work without them.)}
...
We evaluate seven disparate models published online between January 23--26, 2020 that estimated \Ro for the 2019-nCoV outbreak \citep{bedfordncov, imaincov, liuncov, majumderncov, readncov, riouncov, zhaoncov}. 
...
Only propagating error from the growth rate gives very narrow confidence intervals.
Likewise, propagating errors from the growth rate and the mean generation interval gives wider but still too narrow narrow confidence intervals.
...
This procedure allows us to assess the sensitivity of the estimates of \Ro across appropriate ranges of uncertainties and test how underlying assumptions drive the sensitivity.
...
for example, Study 6 may appears to be an outlier but, in fact, the authors explore several different scenarios that allows for changes in reporting rate over time, under which their estimates of \Ro are similar to other estimates.
...
These estimates of the exponential growth rate depend on how incidence has been reported.
...
for example, \cite{zhaoncov} (Study 6) found that that estimates of \Ro could change from 5.47 (95\% CI: 4.16--7.10) to 3.30 (95\% CI: 2.73--3.96) when they assume 2-fold changes in the reporting rate between January 16--20.
...
There are other important phenomena not covered by our simple framework. Examples that seem relevant to this outbreak include: changing reporting rates, changing generation intervals, and reporting delays (including the effects of weekends and holidays.)
\jd{This \P\ should be fleshed out and citated.}
...
For maximal reproducibility, the code should not rely on closed-source software.
\dc{I agree in principle but that's too harsh. People should be free to use whatever they want (Matlab...) as long as they publish the methods and code.}
\bmb{We could leave this out, or phrase it positively (``Methods based on open-source tools allow for maximal reproducibility.'')}

We have provided a basis for evaluating and comparing exponential-growth based estimates of \Ro in terms of three simple components: the exponential growth rate, mean generation interval, and generation interval dispersion. We hope this will help researchers understand and reconcile disparate estimates early in an epidemic.
...
We reviewed 6 modeling studies of the novel coronavirus outbreak that were published online between January 23 and January 26, 2020 (Table 1).
...
\cite{imaincov} and \cite{riouncov} simulated branching process models and compared the predicted number of cases from their models with the estimated number of total cases by January 18.
...
\end{equation}
\end{linenomath*}
We run 4 independent Markov Chain Monte Carlo chains each consisting of 200,000 burnin steps and 200,000 sampling steps.
...
Each chain is represented by a different color.
\bmb{it seems a little weird that the densities of the $\alpha$ parameters are positive at zero. Is this a kernel-density artifact?}
}
\end{figure}
