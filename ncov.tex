...
\begin{flushleft}{
	\Large
	\textbf\newline{
		Reconciling early-outbreak preliminary estimates of the basic reproductive number and its uncertainty: framework and applications to the novel coronavirus (2019-nCoV) outbreak
	}
}
\newline
\\
...
Our results emphasize the importance of propagating uncertainties including the shape of the generation-interval distribution in efforts to estimate \Ro at the outset of an epidemic.
...
We suggest that disease modelers should make sure their assumptions about these three quantities are clear and reasonable, and that estimates of uncertainty of \Ro should propagate error from all three sources \citep{elderd2006uncertainty}.
...
It is also worth noting that all but one study explicitly or implicitly did not assume an exponentially distributed generation interval (i.e., $\kappa=1$), an assumption often used in disease modeling, particularly implicitly (e.g., \cite{may1987transmission, anderson1991infectious, pybus2001epidemic, ferguson2005strategies}).
...
Open triangle: we assumed $\kappa=0.5$ for Study 1 which does not report generation-interval dispersion.
}
...
\jsw{Just pointing out that new data might reveal that the growth rate is no longer exponential which would make an estimate of r mean something altogether different.}
...
Since Study 1 assumes an exponentially distributed generation interval ($\kappa=1$), its estimate of \Ro is relatively insensitive to $r$ and $\bar G$.
...
Here, we used a multi-level Bayesian framework to compare estimates of \Ro for the novel coronavirus outbreak.
...
\jsw{Why is this here? Fine sentence, but belongs 2 paragraphs down and is largely redundant.}
\swp{I proprose we drop these two sentences.}
...
\jsw{Continue to this paragraph which builds on the point of growth rate r}
\swp{No idea what we should continue.}
...
Modelers should be aware of the possibility that ignoring one kind of error can give overly narrow confidence intervals (e.g., \cite{king2015avoidable, taylor2016stochasticity}).
...
We hope this framework will help researchers understand and reconcile disparate estimates of disease strength early in an epidemic.
...
Therefore, we followed recommendations outlined in \citep{gelman2006prior} by reparameterizing the hierarchical model in terms of its mean and standard deviation and imposing a noninformative but proper prior distributions on between-study standard deviations, i.e., $\textrm{half-normal}(0, 10)$.
