...
\end{equation}
\end{linenomath*}
Here, we followed recommendations outlined in \citep{gelman2006prior} by reparameterizing the hierarchical gamma model in terms of its mean and standard deviation and imposing a noninformative but proper prior distributions on between-study standard deviations, i.e., $\textrm{half-normal}(0, 10)$.
We initially imposed gamma priors with small shape parameters ($< 1$) on between-study shape parameters ($=\mu^2/\sigma^2$) of our hierarchical model but such priors can over-penalize the resulting posterior distribution, depending on their shape parameters \citep{gelman2006prior}.
