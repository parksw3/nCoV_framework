...
\begin{flushleft}{
	\Large
	\textbf\newline{
		Reconciling early-outbreak estimates of the basic reproductive number and its uncertainty: framework and applications to the SARS-CoV-2 outbreak
	}
}
\newline
\\
...
However, the estimates of \Ro from different research groups (as well as the associated degrees of uncertainty) vary considerably even though most analyses relied on similar data --- reports of confirmed cases from China, particularly from the Hubei province.
...
Generation-interval dispersiosn represent the squared coefficients of variation in generation intervals.
...
We gathered information on estimates of \Ro and their model assumptions from seven articles that were published online between January 23--26, 2020.
...
This hypothetical example reflects recent scenarios, as increased data availability allows researchers to estimate $r$ with more certainty.
...
As a result, this approach both biases parameter estimates and gives verly narrow confidence intervals \citep{ma2014estimating, king2015avoidable}.
...
since we are primarily focusing on the outbreak in Hubei Province before confinement, generation intervals are unlikely to vary significantly.
...
However, as the number of susceptible individuals decreases or behaviour changes in response to perception of the epidemic, the growth rate will decrease: estimates of $r$ used for \Ro should account for the possibility that $r$ is decreasing through time.
...
Naively comparing estimates of the basic reproductive number without accounting for differences in underlying assumptions can underestimate the differences in the estimates.
