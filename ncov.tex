...
Even though some studies consider reported cases up to Janaury 26, 2020 --- three days after the travel restricion that took place on January 23, 2020 \citep{Tianeabb6105} --- the exponential growth assumption can still describe the number of reported cases reasonably well;
...
In this case, most estimates are primarily driven byreported cases from China, particularly from Wuhan province.
...
The pooled estimates can become sharper as we add more models even when the models no longer add more information about the epidemic.
...
We refer to these estimates as the base estimtes.
...
The assumed distribution in Study 6 does not account for the uncertainty in the meang generation interval $\bar G$, whereas the base estimates acconunt for this uncertainty.
...
We compare the base estimates with \Ro estimates based on the pooled estimates ($\mu_r$, $\mu_G$, $\mu_\kappa$) using \eref{gamma}. 
...
For illustrative purposes, we use our pooled estimates, which may represent a reasonable proxy for the state of knowledge as of January 26, 2020 (\fref{eff}A).
Comparing the estimates that include only some sources of uncertainty to the ``all'' estimate ($\Ro = 3.0$; 95\% CI: 2.1--4.6), we see that propagating error from the growth rate (which all but one of the studies reviewed did) is absolutely crucial: two middle bars ($\mu_G$ and $\mu_\kappa$), which lack growth-rate uncertainty, is relatively narrow.
...
We note that the pooled estimate of the basic reproduction number ($\Ro = 3.0$; 95\% CI: 2.1--4.6) has wider credible intervals than 



\fref{R0} compares the ``base'' estimates (based on $r_i$, $\bar G_i$, and $\kappa_i$ for each study $i$) and 21 ``substitute'' estimates (3 parameter substitutions $\times$ 7 studies).
We find that all but 8 substitute estimates have wider credible intervals compared to their corresponding base estimates --- namely, $\bar G$-substitute estimtates for Study 1 and 7, $r$-substitute estimates for Study 1 and 2, and  $\kappa$-substitute estimates for Study 3, 6, and 7.
We also find that accounting for uncertainties in the estimate of $r$ has the largest effect on the estimates of \Ro\ in most cases (\fref{R0}).
