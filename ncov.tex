...
\jd{no overline, no estimate in abstract.}
...
\jd{Needs to be reworded, but do we still think it's even worth noting?}
...
\jd{Study number doesn't match triangle, also is this clear elsewhere?}
}
...
\jd{We should address issues with exponentiality in the Discussion; nothing is needed here I think.}
...
\jd{Are these really both the same Study 1? I'd like to take a minute to think about this \P.}
...
\jd{Should we lead with the Bayesian fit or the approximation framework? The fit is useful but crude. The approximation is the part I hope will stick.}
...
Here, we used a multilevel Bayesian framework to compare estimates of \Ro for the novel coronavirus outbreak.
...
\jd{Do you really prefer $wk^{-1} to /wk? I never understood that idea.}
...
We initially tried gamma priors with small shape parameters ($< 1$) on between-study shape parameters ($=\mu^2/\sigma^2$) of our hierarchical model but such priors can over-penalize the resulting posterior distribution, depending on their shape parameters \citep{gelman2006prior}.
