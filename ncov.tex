...
Typically, Bayesian hierarchical epidemic modeling seeks to leverage estimates across different outbreaks of a same disease.
In this case, most estimates are based on reported number of cases from China, particularly from Wuhan province; thus, differences in estimates are likely to be driven by differences in the methods and assumptions, rather than epidemiological differences.
In other words, the pooled estimates can become sharper as we add more models even when the models no longer add more information about the epidemic.
