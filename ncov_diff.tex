...
To formalize the estimation of uncertainty at the onset of an outbreak, we present a statistical framework for averaging across estimates of \DIFaddbegin \DIFadd{the }\DIFaddend basic reproductive number \Ro from multiple studies. 
...
Instead, we \DIFdelbegin \DIFdel{decompose estimated }\DIFdelend \DIFaddbegin \DIFadd{model the estimate of }\DIFaddend \Ro \DIFdelbegin \DIFdel{into }\DIFdelend \DIFaddbegin \DIFadd{(as well as the associated generation-interval parameters, $\bar G$, and $\kappa$) from each study with probability distributions that account for the uncertainty in the estimates;
this allows us to re-estimate the corresponding distributions of the exponential growth rates $r$.
We then use a Bayesian multi-level model to average the }\DIFaddend three key quantities ($r$, $\bar G$, and $\kappa$)\DIFdelbegin \DIFdel{and calculate }\DIFdelend \DIFaddbegin \DIFadd{.
The resulting pooled estimates ($\mu_r$, $\mu_G$, and $\mu_\kappa$) are used to calculate the pooled estimate of }\DIFaddend the \DIFdelbegin \DIFdel{average (pooled estimates) of these key parameters.
Calculating }%DIFDELCMD < \Ro %%%
\DIFdel{based on the }\DIFdelend \DIFaddbegin \DIFadd{basic reproduction number, }\Rpool\DIFadd{.
Using }\DIFaddend pooled estimates allows us to average appropriately across the uncertainties present in modeling approaches and in their underlying assumptions.
...
%DIFDELCMD < \begin{tabular}{l|L{2cm}|L{3.2cm}|L{2.8cm}|L{2.5cm}|L{2.4cm}|L{2.7cm}|L{2cm}}
%DIFDELCMD <  & Model & Data (study period) & Data source & Basic reproductive\newline number \Ro & Mean generation\newline interval $\bar G$ (days) & Generation-interval\newline dispersion $\kappa$ & Reference \\
%DIFDELCMD < \hline
%DIFDELCMD < Study 1 & Deterministic branching process model & Total number of cases in Wuhan City, China\newline (Jan 18, 2020) & Estimated by \cite{imaincov0} & 1.5--3.5 & 10 & 1 & \cite{bedfordncov} \\
%DIFDELCMD < \hline
%DIFDELCMD < Study 2 & Stochastic branching process model & Total number of cases in Wuhan City, China\newline (Jan 18, 2020) & Estimated by \cite{imaincov0}  & 2.6 (1.5--3.5)$^\ast$ & 8.4 & Not reported$^\dagger$ & \cite{imaincov} \\
%DIFDELCMD < \hline
%DIFDELCMD < Study 3 & Poisson offspring distribution model & Confirmed cases from China and other countries\newline (Dec 29, 2019--Jan 23, 2020) & Medical records and epidemiological investigations from Guangdong Province, China, and official websites of other regions in China & 2.92 (95\% CI: 2.28--3.67) & 8.4 & 0.2 & \cite{liuncov} \\
%DIFDELCMD < \hline
%DIFDELCMD < Study 4 & Deterministic Metapopulation Susceptible-Exposed-Infected-Recovered (SEIR) model & Confirmed cases from China and other countries\newline (Jan 1--21, 2020) & Not reported & 3.8 (95\% CI: 3.6--4.0) & 7.6 & 0.5 & \cite{readncov} \\
%DIFDELCMD < \hline
%DIFDELCMD < Study 5 & Stochastic branching process model & Total number of cases in Wuhan City, China\newline (Jan 18, 2020) & Estimated by \cite{imaincov0} & 2.2 (90\% CI: 1.4--3.8) & 7--14 & 0.5 & \cite{riouncov} \\
%DIFDELCMD < \hline
%DIFDELCMD < Study 6 & Exponential growth model & Confirmed cases from China\newline (Jan 10--22, 2020) & Wuhan
%DIFDELCMD < Municipal Health Commission, China and National Health Commission of China & 5.47 (95\% CI: 4.16--7.10)$^\ddagger$ & 7.6--8.4 & 0.2 & \cite{zhaoncov} \\
%DIFDELCMD < \hline
%DIFDELCMD < Study 7 & Incidence Decay and Exponential Adjustment (IDEA) model & Reported cases from Wuhan City, China\newline (Dec 1, 2019--Jan 26, 2020) & World	Health Organization, National Health Commission of China, Wuhan Municipal	Health Commission, and \cite{huang2020clinical} & 2.0--3.1 & 6--10 & 0 & \cite{majumderncov} \\
%DIFDELCMD < \hline
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{center}
%DIFDELCMD < \caption{
%DIFDELCMD < \textbf{Summary of the models, analyzed data, reported estimates of the basic reproductive number, and the assumptions about the generation-interval distributions.}
%DIFDELCMD < Model details, estimates of \Ro, and their assumptions about the shape of the generation interval distributions were collected from 7 studies.
%DIFDELCMD < Generation-interval dispersion represent the squared coefficients of variation in generation intervals.
%DIFDELCMD < $^\ast$These intervals reflect \Ro values for best and worst scenarios. We treat these intervals as a 90\% credible interval in our analysis.
%DIFDELCMD < $^\dagger$We assume $\kappa = 0.5$ in our analysis.
%DIFDELCMD < $^\ddagger$The authors presented \Ro estimates under different assumptions regarding the reporting rate; we use their baseline scenario in our analysis to remain consistent with other studies, which do not account for changes in the reporting rate.
%DIFDELCMD < }
%DIFDELCMD < \end{table}
...
\DIFaddbegin \DIFadd{Although this approach nominally treats all studies are equally weighted, the overall pooled estimate will still be weighted by the certainty of the reported estimates (e.g., $r_i$ will be sampled from a narrow distribution and therefore have stronger influence on $\mu_r$ if the reported confidence/credible interval on $r_i$ is narrow).
}\DIFaddend 
...
\DIFaddbegin \DIFadd{The estimates of the between-study standard deviations further suggests that there is a large variability in the underlying parameters among the seven studies, particularly in $r$ and $\kappa$:
...
Points represent medians calculated from the parameter set \DIFdelbeginFL \DIFdelFL{$(\bar{G}_{i}, \kappa_{i}, r_{i})$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$(r_{i}, \bar{G}_{i}, \kappa_{i})$ }\DIFaddendFL for each study $i$ (orange and purple).
Error bars represent 95\% equi-tailed quantiles calculated from the parameter set \DIFdelbeginFL \DIFdelFL{$(\bar{G}_{i}, \kappa_{i}, r_{i})$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$(r_{i}, \bar{G}_{i}, \kappa_{i})$ }\DIFaddendFL for each study $i$.
...
\caption{
  \DIFdelbeginFL \textbf{\DIFdelFL{Effects of $r$, $\bar G$, and $\kappa$ on the estimates of }%DIFDELCMD < \Ro%%%
\DIFdelFL{.}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Effects of the exponential growth rate $r$, mean generation interval $\bar G$, and generation-interval dispersion $\kappa$ on the estimates of the basic reproduction number }\Ro\DIFaddFL{.}}
\DIFaddendFL We compare estimates of \Ro under nine scenarios that propagate different parameter uncertainties (A) based on our pooled estimates ($\mu_r$, $\mu_G$, and $\mu_\kappa$) and (B) assuming a 4-fold reduction in uncertainty of our pooled estimate of the exponential growth rate (using $\hat{\mu}_r = (\mu_r + 3\times\mathrm{median}(\mu_r))/4$ instead of $\mu_r$).
...
Finally, \fref{R0} compares the \DIFaddbegin \DIFadd{reported estimates (Table 1) with the }\DIFaddend base estimates (based on $r_i$, $\bar G_i$, and $\kappa_i$ for each study $i$) \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{as well as }\DIFaddend 21 substitute estimates (3 parameter substitutions $\times$ 7 studies).
\DIFaddbegin \DIFadd{The base estimates, which are probability-based approximations of the reported estimates, are broadly consistent with the reported estimates.
}\DIFaddend All but 8 substitute estimates have wider credible intervals compared to their corresponding base estimates --- the cases with more certain substitute estimates are the $\bar G$-substitute estimates for \DIFdelbegin \DIFdel{Study }\DIFdelend \DIFaddbegin \DIFadd{Studies }\DIFaddend 1\DIFaddbegin \DIFadd{, 5, }\DIFaddend and 7, $r$-substitute estimates for \DIFdelbegin \DIFdel{Study }\DIFdelend \DIFaddbegin \DIFadd{Studies }\DIFaddend 1 and 2, and  $\kappa$-substitute estimates for \DIFdelbegin \DIFdel{Study }\DIFdelend \DIFaddbegin \DIFadd{Studies }\DIFaddend 3, 6, and 7.
...
\DIFdelbegin \DIFdel{We also note that }\DIFdelend \DIFaddbegin \DIFadd{In addition, }\DIFaddend the pooled estimate of the basic reproductive number ($\Rpool = 3.0$; 95\% CI: 2.1--4.6) has wider credible intervals than the base estimates \DIFdelbegin \DIFdel{in all cases }\DIFdelend \DIFaddbegin \DIFadd{for all studies }\DIFaddend except for Study 6.
...
\DIFaddbeginFL \DIFaddFL{The reported estimates refer to estimates listed in Table 1.
}\DIFaddendFL Points represent the medians of \DIFdelbeginFL \DIFdelFL{our }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the reported, }\DIFaddendFL base, substitute, and pooled estimates.
Vertical error bars represent the 95\% credible intervals of our base, substitute, and pooled estimates \DIFaddbeginFL \DIFaddFL{(based on 2000 posterior samples)}\DIFaddendFL .
...
\DIFaddbegin \DIFadd{Emergence of a new strain with different transmissibility could also affect disease dynamics, and complicate inference; this study does not address this possibility.
}\DIFaddend 
...
SWP performed the analysis\DIFaddbegin \DIFadd{, with contributions from all authors}\DIFaddend .
