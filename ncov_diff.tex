...
%DIF < \bibliographystyle{chicago}
%DIF -------
\usepackage[numbers,sort&compress]{natbib} %DIF > 
\bibliographystyle{unsrtnat} %DIF > 
%DIF -------
\date{\today}
...
\DIFaddend A novel coronavirus (2019-nCoV) has recently emerged as a global threat.
...
\DIFdelbegin \DIFdel{Here, we present }\DIFdelend \DIFaddbegin 

\subsection*{\DIFadd{Aim}}

\DIFadd{We aimed to develop }\DIFaddend a framework for comparing and combining different estimates of \Ro across a wide range of models\DIFdelbegin \DIFdel{by decomposing the basic reproductive number }\DIFdelend \DIFaddbegin \DIFadd{.
}

\subsection*{\DIFadd{Methods}}

\DIFadd{We reviewed 7 model-based analyses of the 2019-nCoV outbreak that were published online between January 23--26, 2020.
We decompose their }\Ro \DIFadd{estimates }\DIFaddend into three key quantities: the exponential growth rate $r$, the mean generation interval $\bar G$, and the generation-interval dispersion $\kappa$.
We \DIFdelbegin \DIFdel{then apply our framework to }\DIFdelend \DIFaddbegin \DIFadd{use a Bayesian multilevel model to construct pooled estimates and measure uncertainties associated with these quantities.
}

\subsection*{\DIFadd{Results}}

\DIFadd{We find that most }\DIFaddend early estimates of \Ro \DIFdelbegin \DIFdel{for the 2019-nCoV outbreak.
}\DIFdelend \DIFaddbegin \DIFadd{rely on strong assumptions, especially about the generation-interval dispersion.
...
\DIFaddend Our results emphasize the importance of propagating uncertainties in all components of \Ro, especially the shape of the generation-interval distribution in efforts to estimate \Ro at the outset of an epidemic.

\DIFaddbegin \section*{\DIFadd{Keywords}}

\DIFadd{Basic reproductive number, 2019-nCoV
}

\section*{\DIFadd{Funding}}

\DIFadd{BMB and DJDE were supported by Natural Sciences and Engineering Research Council (NSERC). ML was supported by Canadian Institutes of Health Research (CIHR). The funders had no role in study design, data collection and analysis, decision to publish, or preparation of the manuscript.
}

\section*{\DIFadd{Declaration of interests}}

\DIFadd{We declare no competing interests.
}

\section*{\DIFadd{Acknowledgements}}

\DIFadd{We thank Daihai He for providing helpful comments on the manuscript.
}

\section*{\DIFadd{Contribution}}

\DIFadd{SWP and JD developed the statistical framework. 
...
Since December 2019 \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{pneumonia}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , a novel coronavirus (2019-nCoV) has been spreading in China and other parts of the world\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{pneumonia}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend .
Although the virus is believed to have originated from animal reservoirs\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{cdcncov}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend , the ability of 2019-nCoV \DIFdelbegin \DIFdel{ability }\DIFdelend to directly transmit between humans has posed a greater threat for its spread \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{huang2020clinical,who26report}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{chan2020familial}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
As of February 3, 2020, the World Health Organization (WHO) has confirmed 17391 cases, including 153 confirmed cases in 23 different countries, outside China \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{who02report}}\hspace{0pt}%DIFAUXCMD
.
Also on January 30, 2020, WHO declared the outbreak a Public Health Emergency of International Concern \mbox{%DIFAUXCMD
\citep{whoemer}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
As the disease continues to spread, many researchers have \DIFdelbegin \DIFdel{already published their }\DIFdelend \DIFaddbegin \DIFadd{published their preliminary }\DIFaddend analyses of the outbreak as pre-prints \DIFdelbegin \DIFdel{(e.g., }\DIFdelend \cite{bedfordncov, imaincov, liuncov, majumderncov, readncov, zhaoncov}\DIFdelbegin \DIFdel{) and in peer-reviewed journals (e.g., \mbox{%DIFAUXCMD
\cite{li2020early, riou2020pattern, wu2020nowcasting, zhao2020preliminary}}\hspace{0pt}%DIFAUXCMD
), }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend focusing in particular on estimates of the basic reproductive number \Ro (i.e., the average number of secondary cases generated by a primary case in a fully susceptible population \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{anderson1991infectious, diekmann1990definition}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{anderson1991infectious}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ).
Estimates of the basic reproductive number are of interest during an outbreak because they provide information about the level of intervention required to interrupt transmission\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{anderson1991infectious}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend , and about the \DIFdelbegin \DIFdel{potential }\DIFdelend final size of the outbreak \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
However, it can be difficult to compare a disparate set of estimates of \Ro from different research groups \DIFdelbegin \DIFdel{(as well as the associated degrees of uncertainty) }\DIFdelend when the estimation methods and their underlying assumptions vary widely.
...
Early in an epidemic, information is scarce and there is inevitably a great deal of uncertainty surrounding \DIFdelbegin \DIFdel{both case reports (affecting the estimates of the exponential growth rate) and contact tracing (affecting the estimates of the generation-interval distribution)}\DIFdelend \DIFaddbegin \DIFadd{disease transmission}\DIFaddend .
...
We compare seven disparate models published online between January 23--26, 2020 that estimated \Ro for the 2019-nCoV outbreak \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
the pooled estimates reflect the uncertainties present in modeling approaches\DIFdelbegin \DIFdel{and their underlying assumptions}\DIFdelend .
...
\section{\DIFdelbegin \DIFdel{Results}\DIFdelend \DIFaddbegin \DIFadd{Methods}\DIFaddend }

\DIFaddbegin \subsection{\DIFadd{Description of the studies}}
...
We gathered information on estimates of \Ro and their assumptions about the underlying generation-interval distributions from 7 articles that were published online between January \DIFdelbegin \DIFdel{23 and January 26}\DIFdelend \DIFaddbegin \DIFadd{23--26}\DIFaddend , 2020 (Table 1).
\DIFaddbegin \DIFadd{Five studies \mbox{%DIFAUXCMD
\cite{liuncov, majumderncov, readncov, riouncov, zhaoncov} }\hspace{0pt}%DIFAUXCMD
were uploaded to pre-print servers (bioRxiv, medRxiv, and SSRN); one report was posted on the web site of Imperial College London \mbox{%DIFAUXCMD
\cite{imaincov}}\hspace{0pt}%DIFAUXCMD
; and one report was posted on nextstrain.org \mbox{%DIFAUXCMD
...
, and the Incidence Decay and Exponential Adjustment (IDEA) model \mbox{%DIFAUXCMD
\cite{majumderncov}}\hspace{0pt}%DIFAUXCMD
.
Four studies estimated }\Ro \DIFadd{by directly fitting their models to incidence data \mbox{%DIFAUXCMD
\cite{readncov, zhaoncov, liuncov, majumderncov}}\hspace{0pt}%DIFAUXCMD
.
The remaining three studies estimated }\Ro \DIFadd{by comparing the predicted number of cases from their models with the estimated number of total cases by January 18 (between 1,000 and 9,7000 \mbox{%DIFAUXCMD
\cite{imaincov0}}\hspace{0pt}%DIFAUXCMD
)
Some of these studies have now been published in peer-reviewed journals \mbox{%DIFAUXCMD
\cite{riou2020pattern, zhao2020preliminary} }\hspace{0pt}%DIFAUXCMD
or have been updated with better uncertainty quantification \mbox{%DIFAUXCMD
\cite{readncov2}}\hspace{0pt}%DIFAUXCMD
.
}

\subsection{\DIFadd{Gamma approximation framework for linking $r$ and $\Ro$}}

\DIFadd{Early in an outbreak, }\Ro \DIFadd{is difficult to estimate directly;
instead, }\Ro \DIFadd{is often inferred from the exponential growth rate $r$, which can be estimated reliably from incidence data \mbox{%DIFAUXCMD
\cite{ma2014estimating}}\hspace{0pt}%DIFAUXCMD
.
Given an estimate of the exponential growth rate $r$ and an }\emph{\DIFadd{intrinsic}} \DIFadd{generation-interval distribution $g(\tau)$ \mbox{%DIFAUXCMD
...
\begin{equation}
\DIFadd{1/\Ro = \int \exp(-r\tau) g(\tau) \, d\tau.
\label{eq:euler}
}\end{equation}
\end{linenomath*}
\DIFadd{In other words, estimates of }\Ro \DIFadd{must
depend on the assumptions about the
exponential growth rate $r$ and the shape of the generation-interval distribution $g(\tau)$.
}

\DIFadd{Here, we use the gamma approximation framework \mbox{%DIFAUXCMD
\cite{park2019practical} }\hspace{0pt}%DIFAUXCMD
to (i) characterize the amount of uncertainty present in the exponential growth rates and the shape of the generation-interval distribution and (ii) assess the degree to which these uncertainties affect the estimate of }\Ro\DIFadd{.
...
\begin{equation}
\DIFadd{\Ro = \left(1 + \kappa r \bar{G}\right)^{1/\kappa}.
\label{eq:gamma}
}\end{equation}
\end{linenomath*}
\DIFadd{This equation demonstrates that a generation-interval distribution
that has a larger mean (higher $\bar{G}$) or is less variable (lower $\kappa$)
will give a higher estimate of }\Ro \DIFadd{for the same value of $r$.
}

\subsection{\DIFadd{Statistical framework}}

\DIFaddend As most studies do not report their estimates of the exponential growth rate\DIFdelbegin \DIFdel{or the associated confidence intervals}\DIFdelend , we first recalculate the exponential growth rate that correspond to their model assumptions.
We do so by modeling \DIFdelbegin \DIFdel{explicitly or implicitly }\DIFdelend reported distributions of the reproductive number \Ro, the mean generation interval $\bar G$, and the generation-interval dispersion parameter $\kappa$ with appropriate probability distributions;
...
\DIFdelbegin \DIFdel{We then constructed }\DIFdelend \DIFaddbegin \DIFadd{For each study $i$, we construct }\DIFaddend a family of parameter sets \DIFdelbegin \DIFdel{--- which include $r$,$\bar G$, and $\kappa$ --- for each study and used these in a Bayesian multilevel model to build a distribution of pooled estimates for each parameter ($\mu_r$, $\mu_G$, and $\mu_\kappa$).
The pooled estimates, which are represented as probability distributions rather than point estimates, allow us to average across different modeling approaches, while accounting for the uncertainties in the assumptions they make (see Methods)}\DIFdelend \DIFaddbegin \DIFadd{by drawing 100,000 random samples from the probability distributions (Table 2) that represent the estimates of }\Rx{0i} \DIFadd{and the assumed values of $\bar G_i$ and $\kappa_i$ and calculate the exponential growth rate $r_i$ via the inverse of \eref{gamma}:
}\begin{linenomath*}
\begin{equation}
\DIFadd{r_i = \frac{\Rx{0i}^{\kappa_i} - 1}{\kappa_i \bar{G}_i}.
}\end{equation}
\end{linenomath*}
\DIFadd{This allows us to approximate the probability distributions of the estimated exponential growth rates by each study;
uncertainties in the probability distributions that we calculate for the estimated exponential growth rates will reflect the methods and assumptions that the studies rely on}\DIFaddend .
...
\DIFaddbegin \DIFadd{We construct pooled estimates for each parameter ($r$, $\bar G$, and $\kappa$) using a Bayesian multilevel modeling approach, which assumes that the parameters across different studies come from the same gamma distribution.
...
\end{linenomath*}
\DIFadd{where $\mu_r$, $\mu_G$, $\mu_\kappa$ represent the pooled estimates, and $\sigma_r$, $\sigma_G$, and $\sigma_\kappa$ represent between-study standard deviations.
...
\DIFadd{We use weakly informative priors on hyperparameters:
}\begin{linenomath*}
\begin{equation}
\DIFadd{\begin{aligned}
...
\end{linenomath*}
\DIFadd{We followed recommendations outlined in Gelman }\textit{\DIFadd{et al.}} \DIFadd{\mbox{%DIFAUXCMD
...
We had initially used gamma priors with small shape parameters ($< 1$) on between-study shape parameters ($=\mu^2/\sigma^2$) but found this put too much prior probability on large between-study variances. This phenomenon is a known problem \mbox{%DIFAUXCMD
\cite{gelman2006prior}}\hspace{0pt}%DIFAUXCMD
.
Alternative choices of prior for the between-study shape parameters are also suboptimal: imposing strong priors (e.g. half-Student-$t(\mu=0,\sigma=1,\nu=4)$  assumes }\textit{\DIFadd{a priori}} \DIFadd{that between-study variance is large,  while weak priors (e.g. half-Cauchy(0,5)) can lead to poor mixing.
}

\DIFadd{We run 4 independent Markov Chain Monte Carlo chains each consisting of 500,000 burnin steps and 500,000 sampling steps.
Posterior samples are thinned every 1000 steps.
Convergence is assessed by ensuring that the Gelman-Rubin statistic is below 1.01 for all hyperparameters \mbox{%DIFAUXCMD
...
}\texttt{\DIFadd{R}} \DIFadd{code is available in GitHub (}\url{https://github.com/parksw3/nCoV_framework}\DIFadd{).
}
...
\citep{elderd2006uncertainty}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend %DIF >  Assuming fixed parameter values can lead to overly strong conclusions \cite{elderd2006uncertainty}.
...
Comparing the models that include only some sources of uncertainty to the \DIFdelbegin \textbf{\DIFdel{all}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{``all'' }\DIFaddend model, we see that propagating error from the growth rate (which all but one of the studies reviewed did) is absolutely crucial: the middle bar (\DIFdelbegin \textbf{\DIFdel{GI mean}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{``GI mean''}\DIFaddend ), which lacks growth-rate uncertainty, is relatively narrow.
...
Uncertainty in the generation-interval dispersion also has important effects as it determines the functional form of the relationship between $r$ and \Ro (compare \DIFdelbegin \textbf{\DIFdel{growth rate + GI mean}} %DIFAUXCMD
\DIFdel{with }\textbf{\DIFdel{all}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{``growth rate + GI mean'' with ``all''}\DIFaddend ).
...
This procedure allows us to assess the sensitivity of the estimates of \Ro across appropriate ranges of uncertainties\DIFdelbegin \DIFdel{and test how underlying assumptions affect their estimates}\DIFdelend .
...
We estimate narrower confidence intervals for Study 3, Study 6, and Study 7 when we \DIFdelbegin \DIFdel{use our pooled estimate of }\DIFdelend \DIFaddbegin \DIFadd{account for proper uncertainties in }\DIFaddend the generation-interval dispersion \DIFdelbegin \DIFdel{$\mu_\kappa$ to recalculate }%DIFDELCMD < \Ro %%%
\DIFdelend because they assume a narrow generation-interval distribution (compare \DIFdelbegin \textbf{\DIFdel{base}} %DIFAUXCMD
\DIFdel{with }\textbf{\DIFdel{GI variation}} %DIFAUXCMD
\DIFdel{in \fref{R0}}\DIFdelend \DIFaddbegin \DIFadd{``base'' with ``GI variation''}\DIFaddend );
...
We estimate narrower confidence intervals for Study 5 and Study 7 when we \DIFdelbegin \DIFdel{use our pooled estimate of }\DIFdelend \DIFaddbegin \DIFadd{account for proper uncertainties in }\DIFaddend the mean generation interval \DIFdelbegin \DIFdel{$\mu_G$ to recalculate }%DIFDELCMD < \Ro %%%
\DIFdel{(compare }\textbf{\DIFdel{base}} %DIFAUXCMD
\DIFdel{with }\textbf{\DIFdel{GI mean}} %DIFAUXCMD
\DIFdel{in \fref{R0}}\DIFdelend \DIFaddbegin \DIFadd{(compare ``base'' with ``GI mean''}\DIFaddend ) because the range of uncertainty in the mean generation interval $\bar G$ they consider is much wider than the pooled range (\fref{assumption}).
\DIFdelbegin \DIFdel{We also estimate narrower confidence intervals for Study 1 when we replace their estimated }\DIFdelend \DIFaddbegin \DIFadd{Substituting the reported }\DIFaddend $r$ or $\bar G$ \DIFaddbegin \DIFadd{from Study 1 }\DIFaddend with our pooled estimates \DIFaddbegin \DIFadd{give narrower confidence intervals }\DIFaddend for similar reasons.
...
This is not always a serious problem, particularly if the goal is inferring parameters rather than directly making forecasts \DIFdelbegin \DIFdel{(e.g., \mbox{%DIFAUXCMD
\cite{ma2014estimating}}\hspace{0pt}%DIFAUXCMD
)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ma2014estimating}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
Modelers should however be aware of the possibility that ignoring one kind of error can give overly narrow confidence intervals \DIFdelbegin \DIFdel{(e.
g., \mbox{%DIFAUXCMD
...
for example, \DIFaddbegin \DIFadd{Zhao }\textit{\DIFadd{et al.}} \DIFaddend \cite{zhaoncov} (Study 6) demonstrated that estimates of \Ro can change from 5.47 (95\% CI: 4.16--7.10) to 3.30 (95\% CI: 2.73--3.96) when they assume 2-fold changes in the reporting rate between January 17, when the official diagnostic guidelines were released \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\citep{harris1990reporting, tsuzuki2017dynamics, tariq2019assessing}}\hspace{0pt}%DIFAUXCMD
.
...
%DIFDELCMD < %%%
\DIFdel{In other words, estimates of }%DIFDELCMD < \Ro %%%
\DIFdel{must
depend on the assumptions about the
exponential growth rate $r$ and the shape of the generation-interval distribution $g(\tau)$.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Here, we use the gamma approximation framework \mbox{%DIFAUXCMD
\citep{mcbryde2009early, nishiura2009transmission, roberts2011early, park2019practical} }\hspace{0pt}%DIFAUXCMD
to (i) characterize the amount of uncertainty present in the exponential growth rates and the shape of the generation-interval distribution and (ii) assess the degree to which these uncertainties affect the estimate of }%DIFDELCMD < \Ro%%%
\DIFdel{.
Assuming that generation intervals follow a gamma distribution
with the mean $\bar G$ and the squared coefficient of variation $\kappa$, 
we have
...
.
They vary widely in their statistical methods and in the amount of data used to infer }%DIFDELCMD < \Ro%%%
\DIFdel{.
\mbox{%DIFAUXCMD
\cite{bedfordncov}}\hspace{0pt}%DIFAUXCMD
, \mbox{%DIFAUXCMD
...
simulated branching process models and compared the predicted number of cases from their models with the estimated number of total cases by January 18 (between 1,000 and 9,7000 \mbox{%DIFAUXCMD
...
\mbox{%DIFAUXCMD
\cite{zhaoncov} }\hspace{0pt}%DIFAUXCMD
fitted an exponential growth model to incidence data up to January 22 and inferred }%DIFDELCMD < \Ro %%%
\DIFdel{via the Euler-Lotka equation (\eref{euler}).
\mbox{%DIFAUXCMD
\cite{liuncov} }\hspace{0pt}%DIFAUXCMD
modeled the observed incidence up to January 23 as a function of the observed incidence at previous time steps, the generation-interval distribution, and the basic reproductive number using a Poisson likelihood \mbox{%DIFAUXCMD
\citep{forsberg2008likelihood}}\hspace{0pt}%DIFAUXCMD
.
Finally, \mbox{%DIFAUXCMD
\cite{majumderncov} }\hspace{0pt}%DIFAUXCMD
fitted the Incidence Decay and Exponential Adjustment (IDEA) model \mbox{%DIFAUXCMD
\citep{fisman2013idea} }\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{For each study $i$, we construct a family of parameter sets by drawing 100,000 random samples from the probability distributions (Table 2) that represent the estimates of }%DIFDELCMD < \Rx{0i} %%%
...
We account for uncertainties associated with $r_i$, $\bar G_i$ and $\kappa_i$ (and their correlations), by drawing a random set from the family of parameter sets for each study at each Metropolis-Hastings step;
this approach is analogous to Bayesian methods for analyzing phylogenetic data, which often rely on drawing random samples of phylogenetic trees from a discrete set to account for phylogenetic uncertainty \mbox{%DIFAUXCMD
\citep{pagel2004bayesian,bedford2014integrating}}\hspace{0pt}%DIFAUXCMD
.
Since the gamma distribution does not allow zeros, we use $\kappa=0.02$ instead for Study 7.
We note that this approach does not account for non-independence between the parameter estimates made by different modelers.
...
%DIFDELCMD < \end{linenomath*}
%DIFDELCMD < %%%
\DIFdel{We followed recommendations outlined in \mbox{%DIFAUXCMD
\cite{gelman2006prior}}\hspace{0pt}%DIFAUXCMD
, parameterizing the top-level gamma distributions in terms of their means and standard deviations and imposing weakly informative prior distributions on between-study standard deviations, i.e., $\textrm{half-normal}(0, 10)$.
We had initially used gamma priors with small shape parameters ($< 1$) on between-study shape parameters ($=\mu^2/\sigma^2$) but found this put too much prior probability on large between-study variances. This phenomenon is a known problem \mbox{%DIFAUXCMD
\citep{gelman2006prior}}\hspace{0pt}%DIFAUXCMD
.
Alternative choices of prior for the between-study shape parameters are also suboptimal: imposing strong priors (e.g. half-Student-$t(\mu=0,\sigma=1,\nu=4)$  assumes }\textit{\DIFdel{a priori}} %DIFAUXCMD
...
\DIFdelbegin %DIFDELCMD < \bibliography{ncov}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{ncov_abbr}
\DIFaddend 
