...
To capture the average ranges of uncertainties for each parameter ($r$, $\bar G$, and $\kappa$),
...
\fref{assumption} compares the estimated (or assumed) values of the exponential growth rate $r$, mean generation interval $\bar G$, and the squared CV $\kappa$ of the generation-interval distribution with the average values ($\mu_r$, $\mu_G$, and $\mu_\kappa$) that we calculate from our multilevel model.
...
We estimate narrower confidence intervals when we use the average mean generation time $\mu_G$ to recalculate $\mathcal R_0$ for Study 5 because the range of uncertainty in the mean generation time $\bar G$ that they consider is much wider than ours (\fref{assumption}).
...
When $\mathcal R_0$ is estimated from a wide range of models that rely on different assumptions, it can difficult to assess the statistical validity of these estimates.
Here, we present a simple framework that allows us to evaluate the validity of the estimates of $\mathcal R_0$ and associated model assumptions using the gamma approximation of the generation-interval distribution \citep{park2019practical}. 
We then apply the framework to the preliminary estimates of $\mathcal R_0$ during the novel coronavirus outbreak.
Our results demonstrates the importance of accounting for uncertainties associated with the underlying generation-interval distributions, especially with the amount of dispersion in the generation intervals:
